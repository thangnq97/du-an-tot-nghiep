...
<div>
    ...
    @endif
    <form action="{{ route('bills.store') }}" method="POST" enctype="multipart/form-data" class="row">
        @csrf
        ...
        <div class="mb-3">
         ...
         <select name="room_id" id="" class="form-control ">
                @foreach ($water as $id => $pre_water)
                <option value="{{ $id }}">{{ $id }}--{{ $pre_water }}</option>
                @endforeach
            </select>

        <div class="mb-3">
            <label for="exampleInputPassword1" class="form-label">Tiền phòng</label>
        ...
        <div class="mb-3">
            <label for="exampleInputPassword1" class="form-label">tiền điện</label>
            <input class="form-control " type="number" id="pre_water" name="pre_water" ">
            @error('pre_water')
            <div class="alert alert-danger ">{{ $message }}</div>
            @enderror
        ...
        <div class="mb-3">
            <label for="exampleInputPassword1" class="form-label">Tiềm mước</label>
